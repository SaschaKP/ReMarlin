...
void _menu_temp_filament_op(const PauseMode mode, const int8_t extruder) {
  ...
  #if PREHEAT_COUNT >= 3
    ACTION_ITEM(MSG_PREHEAT_3, [] { _change_filament(ui.material_preset[2].hotend_temp); });
    #if PREHEAT_COUNT >= 4
      ACTION_ITEM(MSG_PREHEAT_4, [] { _change_filament(ui.material_preset[3].hotend_temp); });
      #if PREHEAT_COUNT >= 5
        ACTION_ITEM(MSG_PREHEAT_5, [] { _change_filament(ui.material_preset[4].hotend_temp); });
        #if PREHEAT_COUNT >= 6
          ACTION_ITEM(MSG_PREHEAT_6, [] { _change_filament(ui.material_preset[5].hotend_temp); });
          #if PREHEAT_COUNT >= 7
            ACTION_ITEM(MSG_PREHEAT_7, [] { _change_filament(ui.material_preset[6].hotend_temp); });
          #endif
        #endif
      #endif
    #endif
